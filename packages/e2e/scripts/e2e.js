...
const seleniumStart = seleniumInstall.then(
    () =>
        new Promise((res, rej) => {
            selenium.start(
                {
...
seleniumStart
    .then(child => {
        mkdirp.sync(reports)
        const seleniumLog = fs.createWriteStream(
            path.join(reports, 'selenium.log'),
        )
    ...
    .then(child => {
        ...
        const args = ['--output', reports]

        cp
            .spawn(nightwatchRunner, args, {
                stdio: 'inherit',
            })
            .on('error', err2 => {
                console.error(err2.stack)
